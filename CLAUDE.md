...
| Skill | Scope | Purpose |
|-------|-------|---------|
| `translator` | main | Translate text between languages |
| `excel_normalizer` | document-agent | Normalize Excel files (INN, dates, invisible chars) |
| `meeting_summary` | document-agent | Structure meeting notes into summaries |
| `data_analyst` | data-agent | Analyze data, charts, SQL queries, format conversion |

Skills have a `scope` field: `["main"]` for main agent, `["data-agent"]` for a specific subagent, `["*"]` for all. The SkillMatcher filters by scope before TF-IDF matching.
