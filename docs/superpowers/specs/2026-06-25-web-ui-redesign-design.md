...
**Разведка кода выявила:** thinking-machinery (`ThinkingOverride`/`RequestOptions` contextvar + provider merge) **уже полностью готова** — нужен только carrier-param. `gemma4-fast`/`gemma4-default` sampling profiles уже существуют. `ResearchRuntime.resolve_mode` принимает explicit value, но никто его не передаёт; `dispatch()` — единый choke point.

**Декомпозиция на спринты (вертикальные срезы по фичам, shippable):**

#### Спринт 3A — Fast/Think (thinking depth toggle)
Selector в composer переключает thinking off/on. Осваивает depth_mode plumbing.
- BE core: `depth_mode` param на `AgentLoop.run()` + `AgentRequestService.run()`; loop ставит base `RequestOptions` (Fast→thinking off, Think→default) через `set_request_options`. Переиспользует существующую `ThinkingOverride` machinery (`llm/base.py:100-179`) + provider merge (`openai.py:248-285`). **PhasePolicy остаётся** — его closing-mode override консистентен с Fast.
- BE channel: orchestrator `depth_mode` WS-local var + `depth_mode_change` handler; thread в `service.run(depth_mode=...)`.
- FE: `DepthMode` type (`"fast"|"think"`); selector UI в composer; WS event `depth_mode_change`; hook plumbing. Доступные опции не зависят от section на 3A (Research в 3B).

#### Спринт 3B — Research (force deep research)
Research mode форсит `deep_research` явно, минуя keyword-детекцию.
- BE core: `research_mode` param на `SubagentDispatcher.dispatch` (subagent.py:125); contextvar (параллельный `RequestOptions`) для проброса из `DispatchSubagentTool.execute` БЕЗ изменения LLM-facing tool-schema; thread depth_mode из loop → dispatcher.
- FE: добавить `"research"` в `DepthMode`; selector показывает Research только в Work-разделе (Chat = tools off → research невозможен); UI feedback при активном research (status updates).
