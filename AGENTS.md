...
- **IPC Auth**: HMAC + nonce — **обязательна**, fail-fast при отсутствии `CORPCLAW_IPC_SECRET`. Replay-защита inbound (container→host проверяется host-side) работает через **persistent nonce-store** (`/tmp/corpclaw_nonces.db`, writable tmpfs) — без него каждый `docker exec` стартовал бы с пустым nonce-store и не детектил replay в пределах TTL (H-1). При ошибке открытия файла auth деградирует на in-memory store (verify продолжает работать, но cross-process replay protection отключается).
