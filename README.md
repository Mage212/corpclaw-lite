...
## Опциональные зависимости (Health Endpoint)

Агент поддерживает запуск `HTTP /health` эндпоинта, выдающего служебные метрики (аптайм, запросы, ошибки) в формате JSON, что необходимо при развертывании в Docker Swarm, Kubernetes или других системах оркестрации (Liveness / Readiness probes).

Так как данный функционал нужен не во всех сценариях, легковесный веб-сервер на базе `aiohttp` сделан **опциональной зависимостью** и не устанавливается по умолчанию (чтобы не перегружать базовый агент).

Если вам требуется health-сервер, установите библиотеку в проект вручную:
```bash
uv add aiohttp
```
Встроенные тесты реализуют паттерн *Graceful Degradation* (через `pytest.importorskip`) и автоматически пропустят проверку сервера, если `aiohttp` не установлен, сохраняя сборку зеленой.
