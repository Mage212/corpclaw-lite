...
class BootstrapLoader:
    ...
    def get_user_prompt(self, telegram_id: int) -> str | None:
        """Load per-user prompt generated by onboarding finalization.

        Looks for ``config/bootstrap/users/<telegram_id>.md``.
        Uses the same mtime-based caching as other bootstrap files,
        so changes are picked up automatically on next call.
        """
        path = self._dir / "users" / f"{telegram_id}.md"
        if path.exists():
            content = self._load_cached(path)
            return content.strip() if content.strip() else None
        return None
