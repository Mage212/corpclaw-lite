"""Telegram-specific network helpers with fallback transport.
...
Adapted from Hermes agent (references/hermes-agent/gateway/platforms/telegram_network.py).
"""
...
def _normalize_fallback_ips(values: Iterable[str]) -> list[str]:
    """Validate and normalize fallback IPs (public IPv4 only)."""
    normalized: list[str] = []
    for value in values:
        raw = str(value).strip()
        if not raw:
...
def parse_fallback_ip_env(value: str | None) -> list[str]:
    """Parse comma-separated fallback IPs from environment variable."""
    if not value:
        return []
...
def _rewrite_request_for_ip(request: httpx.Request, ip: str) -> httpx.Request:
    """Rewrite request URL to use fallback IP while preserving Host header and SNI."""
...
def _is_retryable_connect_error(exc: Exception) -> bool:
    """Return True for transient connect errors that warrant a fallback attempt."""
...
class TelegramFallbackTransport(httpx.AsyncBaseTransport):
    ...
    def __init__(self, fallback_ips: Iterable[str], **transport_kwargs: Any) -> None:
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        attempt_order.extend(ip for ip in self._fallback_ips if ip != sticky_ip)
        ...
        for ip in attempt_order:
            ...
            try:
                ...
                if ip is not None and self._sticky_ip != ip:
                    async with self._sticky_lock:
                        if self._sticky_ip != ip:
                            self._sticky_ip = ip
                            logger.warning(
                                "[Telegram] Primary unreachable; using sticky fallback IP %s",
                                ip,
                            )
                return response
            except Exception as exc:
                ...
                if ip is None:
                    logger.warning(
                        "[Telegram] Primary connection failed (%s); trying fallback IPs %s",
                        exc,
