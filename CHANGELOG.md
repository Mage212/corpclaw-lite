...
(D-056: production-ready управление провайдерами/моделями/thinking), но после
живых live-тестов перед тегом выяснилось, что версия несёт ещё две критичные
правки — raw-capture инфраструктуру (PR #33) и фикс контейнерной изоляции,
ломавшей **каждый** tool call в container-режиме. Поэтому major-функционал
D-056 + raw-capture + контейнерный фикс объединены в один выпуск `0.2.1`,
релиз `0.2.0` пропущен.
